...
module.exports = function parsePDFStatement(pdfPath, callback) {
    ...
    statementYear = pdfPath.substr(pdfPath.lastIndexOf(statementFileNameDelimiter)+statementFileNameDelimiter.length,4);
...
function processStatement(text, pageNum) {
    ...
    } while(matches = transactionSeparator.exec(text));
